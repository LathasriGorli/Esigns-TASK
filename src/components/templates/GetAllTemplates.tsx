  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_PUBLIC_API_URL}/api/templates-v2?${new URLSearchParams(queryParams)}`,
        {
          method: 'GET',
          headers: {
        ...
        <table className="border-collapse">
          ...
          <tbody>
            {templates.map((template: any) => (
              <tr key={template.id} className="border-t text-sm">
                <td className={`${TABLE_STYLES.th} w-50`}>{template.title}</td>
                <td className={`${TABLE_STYLES.th} w-30 ${template.status === 'ACTIVE' ? 'text-green-500' : 'text-red-500'}`}>
